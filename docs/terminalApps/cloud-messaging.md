...
1. The businessId + storeId where you want to send your message to (Note: your application running on the terminal can get the storeId by making `IPoyntBusinessService.getBusiness` call. Refer to [this example](authenticating-terminal.html#UsingIPoyntBusinesService)).
